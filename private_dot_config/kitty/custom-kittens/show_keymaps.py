...
Consists of BOTH single keys and key sequences.
"""
# taken from here
...
def handle_result(args: list[str], answer: str, target_window_id: int, boss: Boss):
    ...
    kitty_mod = "+".join(mod_to_names(opts.kitty_mod))
    ...
    for mode in opts.keyboard_modes.values():
        ...
        # because each action can have multiple shortcuts associated with it,
        # we also attempt to group shortcuts with the same actions together.
        for key, action in key_mappings.items():
            key_repr: ShortcutRepr = key.human_repr(kitty_mod=opts.kitty_mod)
            key_repr = key_repr.replace("kitty_mod", kitty_mod)
            key_repr = f"{key_repr:<20} {fmt.fg.red}→{fmt.fg.default}"
